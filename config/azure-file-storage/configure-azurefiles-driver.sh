...
export driver_version="1.19.0"

#Create the azure.json file and upload as secret
wget -nv https://raw.githubusercontent.com/Azure/sterling/$branchName/aro/azure.json -O /tmp/azure.json
envsubst < /tmp/azure.json > /tmp/azure.json
export AZURE_CLOUD_SECRET=`cat /tmp/azure.json | base64 | awk '{printf $0}'; echo`
wget -nv https://raw.githubusercontent.com/Azure/sterling/$branchName/aro/azure-cloud-provider.yaml -O /tmp/azure-cloud-provider.yaml
...
curl -skSL https://raw.githubusercontent.com/kubernetes-sigs/azurefile-csi-driver/$driver_version/deploy/install-driver.sh | bash -s $driver_version --
 ...
 wget -nv https://raw.githubusercontent.com/Azure/sterling/$branchName/aro/azurefiles-standard.yaml -O /tmp/azurefiles-standard.yaml
...
#Deploy premium Storage Class
 wget -nv https://raw.githubusercontent.com/Azure/sterling/$branchName/aro/azurefiles-premium.yaml -O /tmp/azurefiles-premium.yaml
...
sudo -E /tmp/OCPInstall/oc apply -f https://raw.githubusercontent.com/Azure/sterling/$branchName/aro/persistent-volume-binder.yaml